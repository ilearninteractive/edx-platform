...
def get_course_assignments(course_key, user, request, include_access=False):
    ...
    Assignment = namedtuple('Assignment', ['block_key', 'title', 'url', 'date', 'requires_full_access'])
    assignments = []
    for (block_key, date_type), date in all_course_dates.items():
        ...
        assignments.append(Assignment(block_key, title, url, date, requires_full_access))
